...
Argus V1 is the supported supervised release-candidate pipeline, with alpha runtime maturity. V2 agent, continuous, and evidence-graph modules are explicitly gated experimental scaffolding, not a supported continuous service.
...
- **Operator-selected analysis.** Cloud Claude is only for approved non-sensitive data; local Ollama avoids cloud egress; the heuristic engine is offline. Local processing is not a compliance guarantee.
...
The bounded V1 planner proposes a profile; the guardrail authorizes it before a collector executes. This is operator-invoked, not unattended autonomy.
...
Current release-closeout collection: **288 tests** on Python 3.12. Older totals in `BUILD_AND_TEST_LOG.md` are retained as labeled historical snapshots.
...
- [`../docs/index.html`](../docs/index.html) — generated release control dashboard
- [`../docs/control/PROJECT_CONTROL.md`](../docs/control/PROJECT_CONTROL.md) — canonical project control and claim matrix
