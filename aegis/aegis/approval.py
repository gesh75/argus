...
A token authorizes ONE mode (e.g. ``local``) against an EXACT set of targets until a
hard expiry. It is an HMAC over ``(mode, canonical-sorted-targets, expiry)`` keyed by
the audit signing key, so:
...
def _body(mode: str, targets: list[str], expiry: int) -> str:
    return f"v1|{mode}|{_canon_targets(targets)}|{expiry}"


def mint(mode: str, targets: list[str], key: str, *, ttl: int = 3600,
         now: float | None = None) -> str:
    """Mint a base64 approval token authorizing `mode` against `targets` for `ttl` seconds."""
    if ttl <= 0:
        raise ApprovalError("ttl must be positive")
    expiry = int((time.time() if now is None else now) + ttl)
    mac = hmac.new(key.encode(), _body(mode, targets, expiry).encode(), hashlib.sha256).hexdigest()
    return base64.urlsafe_b64encode(f"{expiry}.{mac}".encode()).decode()


def verify(token: str, mode: str, targets: list[str], key: str,
           *, now: float | None = None) -> None:
    """Raise ApprovalError unless `token` authorizes exactly `mode`+`targets` and is unexpired."""
    if not token:
        raise ApprovalError(f"{mode} mode requires an approval token (mint one with `aegis approve`)")
    try:
    ...
    expect = hmac.new(key.encode(), _body(mode, targets, expiry).encode(), hashlib.sha256).hexdigest()
